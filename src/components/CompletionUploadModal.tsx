...
export default function CompletionUploadModal({ task, users, currentUserId, onClose, onComplete }: CompletionUploadModalProps) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    onComplete(task.id, note.trim() || '완료 증빙이 등록되었습니다.', attachment);
    setNote('');
    setFile(null);
  };
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-lg rounded-lg bg-white p-5 shadow-2xl dark:bg-slate-900">
        ...
        <label className="block">
          ...
          <textarea
            ...
            className="mt-2 w-full resize-none rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-emerald-500 dark:border-slate-700 dark:bg-slate-800"
          />
        </label>
        ...
        <label className="mt-4 block">
          ...
          <input
            ...
            className="mt-2 w-full rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-4 text-sm font-semibold dark:border-slate-700 dark:bg-slate-800"
          />
        </label>

        <div className="mt-6 flex gap-2">
          <button type="button" onClick={onClose} className="flex-1 rounded-lg border border-slate-200 px-4 py-3 text-sm font-black text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200">
            취소
          </button>
          <button type="submit" className="flex-1 rounded-lg bg-emerald-600 px-4 py-3 text-sm font-black text-white hover:bg-emerald-700">
            업로드하고 완료
          </button>
        </div>
      </form>
