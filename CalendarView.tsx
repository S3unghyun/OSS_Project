...
import { Calendar, ChevronLeft, ChevronRight, Plus, Info, Check, Clock, Flame, AlertCircle } from 'lucide-react';
...
export default function CalendarView({
  ...
  // Setup year-month state to transition between months. Default is June 2026.
  ...
  // Math helper for calendaring
  const getDaysInMonth = (y: number, m: number) => {
    return new Date(y, m + 1, 0).getDate();
  };

  const getFirstDayOfMonth = (y: number, m: number) => {
    return new Date(y, m, 1).getDay(); // 0 = Sunday, 6 = Saturday
  };

  // Build preceding days (padding from previous month)
  ...
  for (let i = firstDayOfWeek - 1; i >= 0; i--) {
    ...
    days.push({
      ...
      dayOfWeek: new Date(prevYear, prevMonth, dNum).getDay()
    });
  }
  ...
  // Next Month padding
  const totalCellsParsed = days.length;
  const trailingCellsNeeded = totalCellsParsed % 7 === 0 ? 0 : 7 - (totalCellsParsed % 7);
  ...
  for (let n = 1; n <= trailingCellsNeeded; n++) {
    ...
    days.push({
      ...
      dateStr: dStr,
      isCurrentMonth: false,
      dayOfWeek: new Date(nextYear, nextMonth, n).getDay()
    });
  }
  ...
  // Get tasks matching the selected agenda date
  const selectedAgendaTasks = tasks.filter(task => task.dueDateValue === selectedDate);

  // Stats summaries for current display month
    ...
    <div className="space-y-5 pb-28">
      ...
      <header className="sticky top-0 z-30 flex justify-between items-center bg-surface-bg/85 backdrop-blur-md py-4 w-full">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-extrabold text-primary tracking-tight">할 일 달력</h1>
        </div>
        <button
      ...
      <section className="bg-surface-container-low p-4 rounded-2xl border border-outline-variant/20 shadow-sm space-y-4">
        <div className="flex justify-between items-center">
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-surface-container-high transition-colors text-on-surface-variant hover:text-on-surface active:scale-90 cursor-pointer"
            title="이전 달"
          >
        ...
        {/* Small Progress Indicators for the displayed month */}
        <div className="grid grid-cols-2 gap-3 text-center border-t border-outline-variant/10 pt-3">
          <div className="bg-surface-container-lowest p-2 rounded-xl border border-outline-variant/5">
            <span className="text-[10px] text-outline font-extrabold block uppercase tracking-wider">이달 완료한 일</span>
            <span className="text-sm font-extrabold text-success-emerald mt-0.5 block">{monthCompleted}개</span>
          </div>
          <div className="bg-surface-container-lowest p-2 rounded-xl border border-outline-variant/5">
            <span className="text-[10px] text-outline font-extrabold block uppercase tracking-wider">이달 남은 과제</span>
        ...
        <div className="space-y-1">
          ...
          <div className="grid grid-cols-7 gap-1 pt-1.5">
            {days.map((day, idx) => {
              ...
              // Color configuration for day of week text
              let textDayColor = 'text-on-surface';
              if (!belongsToThisMonth) {
                textDayColor = 'text-on-surface-variant/30 dark:text-neutral-700/50';
              } else if (day.dayOfWeek === 0) {
                textDayColor = 'text-red-500 font-extrabold';
              } else if (day.dayOfWeek === 6) {
                textDayColor = 'text-blue-500 font-extrabold';
              }
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center w-full">
                    {/* Day string index indicator */}
                    <span className={`text-[11px] font-extrabold ${textDayColor}`}>
                      {day.dayNum}
                    </span>
                    
                    {/* If selected date matches today, paint a visual focus dot */}
                    {isToday && (
                      <span className="w-1.5 h-1.5 bg-primary rounded-full ring-2 ring-primary-container shrink-0"></span>
                    )}
                  </div>

                  {/* Task Counter badge: e.g. "할 일 +1" or green check tag */}
                  <div className="w-full flex flex-col gap-0.5 justify-end items-center mt-1">
                    {incompleteOnDay > 0 ? (
                      <div className="text-[8px] px-1 py-0.5 leading-none rounded font-extrabold uppercase bg-red-100 text-red-700 hover:bg-red-200 border border-red-200 max-w-full truncate text-center scale-95" title={`미완료 할 일 ${incompleteOnDay}개`}>
                        할 일 +{incompleteOnDay}
                      </div>
                    ) : completedOnDay > 0 ? (
                      <div className="text-[8px] px-1 py-0.5 leading-none rounded font-extrabold uppercase bg-success-container text-success-emerald border border-success-emerald/10 max-w-full truncate text-center scale-95" title={`완료된 할 일 ${completedOnDay}개`}>
                        완료 ✓
                      </div>
                    ) : null}
                  </div>
      ...
      {/* Selected Date Agenda List Section */}
      <section className="space-y-3">
        <div className="flex justify-between items-center px-1">
          <div className="space-y-0.5">
            <span className="text-[10px] font-extrabold text-outline uppercase tracking-widest block">SELECTED DATE AGENDA</span>
            <h3 className="text-sm font-bold text-on-surface">
              {selectedDate === CURRENT_DATE_STR ? "오늘 " : ""}
              {parseInt(selectedDate.split('-')[1])}월 {parseInt(selectedDate.split('-')[2])}일 할 일 목록
            </h3>
          </div>
          <button
        ...
        <div className="space-y-2.5">
          {selectedAgendaTasks.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center p-8 bg-surface-container-low/50 border border-dashed border-outline-variant/20 rounded-2xl text-center space-y-3">
              ...
              <div className="space-y-1">
                <p className="text-xs font-bold text-on-surface-variant">이 날짜에 등록된 일정이 없습니다.</p>
                <p className="text-[10px] text-outline">할 일 가득 찬 하루를 채우기 위해 일정을 등록해 보세요!</p>
              </div>
              <button
                onClick={() => onOpenNewTaskModalWithDate(selectedDate)}
